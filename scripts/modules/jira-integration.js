...
import { extractRefIdFromTitle, formatTitleForJira, getRefId } from './reference-id-service.js';
...
function validateJiraConfig(explicitRoot = null) {
  ...
  if (!baseUrl || baseUrl.includes('{{') || 
      ...
      !apiToken || apiToken.includes('{{')) {
    log('warn', 'Jira configuration contains placeholder values. Please update your .taskmasterconfig file.');
    return null;
  }
...
${taskData.details}`
    ...
    if (!response.ok) {
      ...
      log('error', `Error creating Jira user story: ${JSON.stringify(errorData)}`);
      return null;
    }
...
async function createSubtask(subtaskData, parentKey, explicitRoot = null) {
  ...
  if (!parentExists) {
    log('error', `Parent issue ${parentKey} does not exist in Jira. Cannot create subtask.`);
    return null;
  }
...
${subtaskData.details}`
    ...
    for (const typeName of subtaskTypeNames) {
      try {
        response = await fetch(`${baseUrl}/rest/api/${JIRA_API_VERSION}/issue`, {
          method: 'POST',
          headers: {
            'Authorization': `Basic ${Buffer.from(`${email}:${apiToken}`).toString('base64')}`,
            'Accept': 'application/json',
        ...
        if (response.ok) {
          success = true;
          log('info', `Subtask created successfully using issue type name: ${typeName}`);
          break;
        } else {
          errorData = await response.json();
          log('info', `Attempted with issue type '${typeName}', got error: ${JSON.stringify(errorData)}`);
        }
      } catch (typeError) {
        log('info', `Error trying issue type '${typeName}': ${typeError.message}`);
      }
    }
    
    if (!success) {
      log('error', `Error creating Jira subtask after trying all issue type names: ${JSON.stringify(errorData)}`);
      return null;
    }
...
async function createMissingSubtask(issueKey, taskData, explicitRoot = null) {
  ...
  if (!parentExists && taskData.parentId && taskData.tasksData && taskData.tasksData.tasks) {
    try {
      const parentTask = taskData.tasksData.tasks.find(t => t.id === taskData.parentId);
      if (parentTask && parentTask.metadata && parentTask.metadata.jiraKey) {
        ...
        if (parentExists) {
          ...
          log('info', `Found parent issue with new key ${parentKey} instead of ${taskData.parentJiraKey}`);
        }
      }
      ...
      if (!parentExists && parentTask) {
        ...
        if (parentRefId) {
          ...
          const parentIssueKey = await findIssueKeyByRefId(parentRefId, explicitRoot);
          if (parentIssueKey) {
            parentKey = parentIssueKey;
            parentExists = true;
            log('info', `Found parent issue with key ${parentKey} by reference ID ${parentRefId}`);
          }
        }
      }
    } catch (e) {
  ...
  if (parentExists) {
    ...
    log('error', `Parent issue not found with key ${taskData.parentJiraKey} or any new key. Cannot create subtask.`);
    return null;
  }
}
...
async function findIssueKeyByRefId(refId, explicitRoot = null) {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/rest/api/${JIRA_API_VERSION}/search?jql=${encodeURIComponent(jql)}`, {
      method: 'GET',
      headers: {
        'Authorization': `Basic ${Buffer.from(`${email}:${apiToken}`).toString('base64')}`,
        'Accept': 'application/json'
    ...
    if (!response.ok) {
      const errorData = await response.json();
      log('error', `Error searching for Jira issue by reference ID: ${JSON.stringify(errorData)}`);
      return null;
    }
    ...
    log('error', `Error searching for Jira issue by reference ID: ${error.message}`);
    return null;
  }
}
...
async function issueExists(issueKey, explicitRoot = null) {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/rest/api/${JIRA_API_VERSION}/issue/${issueKey}`, {
      method: 'GET',
      headers: {
        'Authorization': `Basic ${Buffer.from(`${email}:${apiToken}`).toString('base64')}`,
        'Accept': 'application/json'
 ...
 */
async function createIssueLink(fromIssueKey, toIssueKey, linkType, explicitRoot = null) {
  const config = validateJiraConfig(explicitRoot);
  ...
  try {
    const response = await fetch(`${baseUrl}/rest/api/${JIRA_API_VERSION}/issueLink`, {
      method: 'POST',
      headers: {
        'Authorization': `Basic ${Buffer.from(`${email}:${apiToken}`).toString('base64')}`,
        'Accept': 'application/json',
    ...
    if (response.status === 404) {
      ...
      if (effectiveLinkType !== 'Relates') {
        log('warn', `Link type '${effectiveLinkType}' not found. Trying with 'Relates'...`);
        return await createIssueLink(fromIssueKey, toIssueKey, 'Relates', explicitRoot);
      }
    }
    ...
    log('success', `Created link from ${fromIssueKey} to ${toIssueKey} with type ${effectiveLinkType}`);
    return true;
  } catch (error) {
 ...
 * @returns {Promise<boolean>} True if successful, false otherwise
 */
async function updateIssueStatus(issueKey, status, explicitRoot = null, taskData = null) {
  ...
  try {
    ...
    if (!exists) {
      log('warn', `Jira issue ${issueKey} does not exist. Attempting to create it...`);
      
      if (!taskData) {
        log('error', `Cannot create Jira issue ${issueKey}: No task data provided.`);
        return false;
      }
      
      // Create the issue in Jira
      const newIssueKey = await createMissingJiraIssue(issueKey, taskData, explicitRoot);
      if (!newIssueKey) {
        return false;
      }
    ...
    const transitionsResponse = await fetch(`${baseUrl}/rest/api/${JIRA_API_VERSION}/issue/${issueKey}/transitions`, {
      method: 'GET',
      headers: {
        'Authorization': `Basic ${Buffer.from(`${email}:${apiToken}`).toString('base64')}`,
        'Accept': 'application/json'
    ...
    if (!transition) {
      log('warn', `No transition found for status '${status}' (mapped to '${jiraStatus}'). Available transitions: ${transitionsData.transitions.map(t => t.name).join(', ')}`);
      return false;
    }
    
    // Execute the transition
    const updateResponse = await fetch(`${baseUrl}/rest/api/${JIRA_API_VERSION}/issue/${issueKey}/transitions`, {
      method: 'POST',
      headers: {
        'Authorization': `Basic ${Buffer.from(`${email}:${apiToken}`).toString('base64')}`,
        'Accept': 'application/json',
 ...
 */
async function updateJiraTaskStatus(taskId, newStatus, data, tasksPath, options = {}) {
	...
	log('info', `Jira integration is enabled. Updating issue status for task ${taskId}...`);
	...
	if (!jiraKey) {
		...
		log('info', `Task ${taskId} - Reference ID: ${refId || 'NONE'}, Title: ${task.title || 'UNTITLED'}`);
		if (refId) {
			log('info', `No Jira key found in metadata. Searching for issue by reference ID ${refId}...`);
			...
			if (jiraKey) {
				...
				options.writeJSON?.(tasksPath, data) || (typeof writeJSON === 'function' && writeJSON(tasksPath, data));
			}
		}
	}
	...
	if (jiraKey) {
		...
		log('warn', `No Jira key found for task ${taskId}. Skipping Jira status update.`);
	}
	
	// Also update any subtasks if this is a parent task
	if (task && Array.isArray(task.subtasks) && task.subtasks.length > 0) {
		log('info', `Updating Jira status for ${task.subtasks.length} subtasks of task ${taskId}...`);
		...
		for (const subtask of task.subtasks) {
			...
			if (!subtaskJiraKey) {
				...
				log('info', `Subtask ${subtask.id} - Reference ID: ${subtaskRefId || 'NONE'}, Title: ${subtask.title || 'UNTITLED'}`);
				if (subtaskRefId) {
					log('info', `No Jira key found in metadata for subtask ${subtask.id}. Searching for issue by reference ID ${subtaskRefId}...`);
					subtaskJiraKey = await findIssueKeyByRefId(subtaskRefId, projectRoot);
					if (subtaskJiraKey) {
						log('success', `Found Jira issue ${subtaskJiraKey} by reference ID ${subtaskRefId}`);
			...
			if (subtaskJiraKey) {
				try {
					...
					const success = await updateIssueStatus(subtaskJiraKey, newStatus, projectRoot, subtaskData);
					if (success) {
						log('success', `Updated Jira issue ${subtaskJiraKey} status for subtask ${subtask.id}`);
					} else {
						log('warn', `Failed to update Jira issue ${subtaskJiraKey} status for subtask ${subtask.id}`);
					}
				} catch (jiraError) {
					log('error', `Error updating Jira issue status for subtask ${subtask.id}: ${jiraError.message}`);
				}
			} else {
				log('warn', `No Jira key found for subtask ${subtask.id}. Skipping Jira status update.`);
			}
		}
	}
}
