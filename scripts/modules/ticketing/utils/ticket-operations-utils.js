...
export async function createTicketForTask(task, ticketingSystem, options = {}) {
	...
	try {
		...
		if (!ticketingSystem) {
			logger.error(
				'Cannot create ticket: ticketing system is null or undefined'
			);
			return null;
		}
		...
		if (existingTicketId) {
			if (debug) {
				logger.info(
					`Task ${task.id} already has ticket ID: ${existingTicketId}`
				);
			}

			// Verify that the ticket exists in the ticketing system
			const exists = await ticketingSystem.ticketExists(
				existingTicketId,
				projectRoot
			);
			if (exists) {
				if (debug) {
					logger.info(
						`Ticket ${existingTicketId} already exists in the ticketing system`
					);
				}
				return existingTicketId;
			} else {
				logger.warn(
					`Ticket ${existingTicketId} does not exist in the ticketing system, will create a new one`
				);
			}
		}
		...
		if (isSubtask) {
			...
			logger.error(
				`Cannot create ticket for subtask ${task.id} directly, parent task ticket must be created first`
			);
			return null;
		}
		...
		if (!ticketId) {
			logger.error(
				`Created ticket for task ${task.id}, but no ticket ID was returned`
			);
			return null;
		}
		...
		logger.error(
			`Error creating ticket for task ${task?.id || 'unknown'}: ${error.message}`
		);
		return null;
	}
}
 ...
 */
export async function createSubtaskTicket(
	subtask,
	parentTask,
	ticketingSystem,
	options = {}
	...
	try {
		if (!subtask) {
			logger.error(
				'Cannot create subtask ticket: subtask is null or undefined'
			);
			return null;
		}
		...
		if (!parentTask) {
			logger.error(
				'Cannot create subtask ticket: parent task is null or undefined'
			);
			return null;
		}
		...
		if (!ticketingSystem) {
			logger.error(
				'Cannot create subtask ticket: ticketing system is null or undefined'
			);
			return null;
		}
		...
		if (existingTicketId) {
			if (debug) {
				logger.info(
					`Subtask ${subtask.id} already has ticket ID: ${existingTicketId}`
				);
			}
		...
		if (!parentTicketId) {
			logger.error(
				`Cannot create subtask ticket: parent task ${parentTask.id} does not have a ticket ID`
			);
			return null;
		}
		...
		logger.info(
			`Creating subtask ticket for subtask ${subtask.id}: ${subtask.title} under parent ${parentTask.id} (${parentTicketId})`
		);
		...
		if (!ticketId) {
			logger.error(
				`Created subtask ticket for subtask ${subtask.id}, but no ticket ID was returned`
			);
			return null;
		}
		...
		logger.success(
			`Created subtask ticket ${ticketId} for subtask ${subtask.id}`
		);
		return ticketId;
	} catch (error) {
		logger.error(
			`Error creating subtask ticket for subtask ${subtask?.id || 'unknown'}: ${error.message}`
		);
		return null;
	}
}
