...
export async function getAllTickets(config) {
	...
	try {
		...
		if (!response.ok) {
			...
			log(
				'error',
				`Error fetching Jira tickets: ${response.status} ${errorText}`
			);
			return [];
		}
...
export async function getTicketStatus(ticketId, config) {
	...
	try {
		...
		if (!response.ok) {
			...
			log(
				'error',
				`Error fetching Jira ticket status: ${response.status} ${errorText}`
			);
			return null;
		}
...
export async function updateTicketStatus(ticketId, jiraStatus, config) {
	...
	try {
		...
		if (!transitionsResponse.ok) {
			...
			log(
				'error',
				`Error fetching Jira transitions: ${transitionsResponse.status} ${errorText}`
			);
			return false;
		}
		...
		if (!updateResponse.ok) {
			...
			log(
				'error',
				`Error updating Jira status: ${updateResponse.status} ${errorText}`
			);
			return false;
		}
...
export async function createIssue(issueData, config) {
	...
	for (const key in sanitizedIssueData.fields) {
		if (typeof key === 'object') {
			log(
				'warn',
				`Removing invalid field with object key: ${JSON.stringify(key)}`
			);
	...
	try {
		...
		if (!response.ok) {
			...
			log(
				'error',
				`Error creating Jira issue: ${response.status} ${errorText}`
			);
...
export async function deleteTicket(ticketId, config) {
	...
	try {
		...
		if (response.status === 204) {
			...
			log(
				'error',
				`Failed to delete Jira ticket ${ticketId}: ${response.status} ${errorText}`
			);
			return false;
		}
	} catch (error) {
