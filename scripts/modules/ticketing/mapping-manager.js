...
function loadMappingConfig(ticketingSystem = null, explicitRoot = null) {
  ...
  if (mappingFilePath && fs.existsSync(mappingFilePath)) {
    try {
      ...
      log('info', `Loaded custom ${systemType} mapping configuration from ${mappingFilePath}`);
    } catch (error) {
      log('error', `Failed to parse ${systemType} mapping configuration: ${error.message}`);
      log('warn', `Using default ${systemType} mapping configuration`);
    }
  } else {
    log('debug', `No custom ${systemType} mapping configuration found, using defaults`);
  }
 ...
 */
function saveMappingConfig(ticketingSystem, mappingConfig, explicitRoot = null) {
  if (!ticketingSystem || !mappingConfig) {
    log('error', 'Cannot save mapping config: missing ticketing system or mapping data');
    return false;
  }
  
  const mappingDir = getMappingConfigDir(explicitRoot);
  if (!mappingDir) {
    log('error', 'Cannot save mapping config: unable to determine project root');
    return false;
  }
  ...
  const mappingFilePath = path.join(mappingDir, `${ticketingSystem}-mapping.json`);
  ...
  try {
    ...
    log('success', `Saved ${ticketingSystem} mapping configuration to ${mappingFilePath}`);
    ...
    log('error', `Failed to save ${ticketingSystem} mapping configuration: ${error.message}`);
    return false;
  }
}
...
function initializeDefaultMappings(explicitRoot = null) {
  const mappingDir = getMappingConfigDir(explicitRoot);
  if (!mappingDir) {
    log('error', 'Cannot initialize mappings: unable to determine project root');
    return;
  }
  ...
  Object.keys(DEFAULT_MAPPINGS).forEach(system => {
    ...
    if (!fs.existsSync(mappingFilePath)) {
      try {
        ...
        log('info', `Created default ${system} mapping configuration at ${mappingFilePath}`);
      } catch (error) {
        log('error', `Failed to create default ${system} mapping: ${error.message}`);
      }
    }
  });
}
 ...
 */
function getIssueTypeMapping(taskType, ticketingSystem = null, explicitRoot = null) {
 ...
 */
function getFieldMapping(fieldName, fieldValue, ticketingSystem = null, explicitRoot = null) {
  const mapping = loadMappingConfig(ticketingSystem, explicitRoot);
  ...
  const mappedValue = fieldConfig.mapping[fieldValue] || 
 ...
 */
function shouldIgnoreField(fieldName, ticketingSystem = null, explicitRoot = null) {
  const mapping = loadMappingConfig(ticketingSystem, explicitRoot);
 ...
 */
function getRelationshipMapping(relationType, ticketingSystem = null, explicitRoot = null) {
  const mapping = loadMappingConfig(ticketingSystem, explicitRoot);
