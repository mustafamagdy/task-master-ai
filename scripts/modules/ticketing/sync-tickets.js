...
import { getRefId, formatTitleForTicket } from './utils/index.js';
import {
	getTicketId,
	storeTicketId,
...
async function syncTickets(tasksPath, options = {}) {
	...
	try {
		...
		const synchronizeTaskStatus = async (
			taskItem,
			ticketIdentifier,
			isSubtaskItem
		) => {
			try {
				debugLog(
					`Synchronizing status for ${isSubtaskItem ? 'subtask' : 'task'} ${taskItem.id} with ticket ${ticketIdentifier}...`
				);

				if (!ticketingSystem) {
					customLog.error(
						`Ticketing system is undefined for ${isSubtaskItem ? 'subtask' : 'task'} ${taskItem.id}`
					);
					return false;
				}
				...
				if (!ticketIdentifier) {
					customLog.error(
						`Ticket identifier is undefined for ${isSubtaskItem ? 'subtask' : 'task'} ${taskItem.id}`
					);
					return false;
				}
				...
				const result = await syncTaskStatus(
					...
					ticketingSystem,
					{
						...
						onStatusChanged: (task, newStatus, originalTicketStatus) => {
							customLog.success(
								`Updated ${isSubtaskItem ? 'subtask' : 'task'} ${task.id} status from "${task.status}" to "${newStatus}" (original ticket status: "${originalTicketStatus}")`
							);
				...
				if (!result) {
					debugLog(
						`No status change needed for ${isSubtaskItem ? 'subtask' : 'task'} ${taskItem.id}`
					);
				}
		...
		for (const task of data.tasks) {
			...
			try {
				...
				if (!refId) {
					customLog.warn(`Task ${task.id} has no reference ID, skipping`);
					continue;
				}
				...
				debugLog(
					`Task ${task.id} initial ticketId check: ${ticketId || 'undefined'}`
				);
				...
				if (task.subtasks && task.subtasks.length > 0 && ticketId) {
					...
					if (!parentTaskTicketId) {
						customLog.error(
							`Synchronization error: parentTicketId is not defined`
						);
						customLog.warn(
							`Ticket synchronization skipped: Error: parentTicketId is not defined`
						);
					} else {
						...
						for (const subtask of task.subtasks) {
							try {
								...
								let subtaskTicketId = getTicketId(subtask, {
									parentTask: task,
									debug
								});
								...
								if (subtaskTicketId) {
									debugLog(
										`Synchronizing status for subtask ${subtask.id} with ticket ${subtaskTicketId}...`
									);
								...
								if (!subtaskTicketId) {
									customLog.info(
										`Creating new subtask ticket for subtask ${subtask.id} under parent ${ticketId}...`
									);
									try {
										// Use the centralized subtask ticket creation utility
										subtaskTicketId = await createSubtaskTicket(
											subtask,
											task,
											ticketingSystem,
											{
										...
										if (subtaskTicketId) {
											...
											customLog.error(
												`Failed to create subtask ticket for subtask ${subtask.id}`
											);
											stats.errors++;
										}
									} catch (error) {
				...
				if (finalTicketId) {
					...
					customLog.warn(
						`Skipping main task sync for task ${task.id} - No ticketId available in metadata`
					);
				}
			} catch (taskError) {
				customLog.error(
