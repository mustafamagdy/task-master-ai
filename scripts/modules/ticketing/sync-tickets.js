 ...
 */
async function fetchAllTicketsFromSystem(
	ticketingSystem,
	projectRoot,
	logger,
	debugLog
	...
	try {
		...
		const task = {
			...
			status: ticketingSystem.mapTicketStatusToTaskmaster
				? ticketingSystem.mapTicketStatusToTaskmaster(ticket.status)
				: 'pending',
			priority: ticketingSystem.mapTicketPriorityToTaskmaster
				? ticketingSystem.mapTicketPriorityToTaskmaster(ticket.priority)
				: 'medium',
			metadata: {
...
async function syncTickets(tasksPath, options = {}) {
	...
	try {
		...
		return {
			success: false,
			message: `Error getting ticketing system: ${error.message}`
		};
	}
	...
	if (!force && !enabled) {
		...
		return {
			success: false,
			message:
				'Ticketing system integration is not enabled. Use --force to override or update your .taskmasterconfig file to enable ticketing integration.'
		};
	}
	...
	try {
			...
			try {
				...
				if (ticketId) {
					try {
						...
						if (!ticketExists) {
							customLog.warn(
								`Ticket ${ticketId} referenced in task ${task.id} does not exist in Jira. Will recreate.`
							);
				 ...
				 */
				async function synchronizeTaskStatus(
					taskItem,
					ticketIdentifier,
					isSubtaskItem
				) {
					debugLog(
						`Syncing status for ${isSubtaskItem ? 'subtask' : 'task'} ${taskItem.id} with ticket ${ticketIdentifier}`
					);
					...
					try {
						...
						const jiraStatusInTaskmaster =
							ticketingSystem.mapTicketStatusToTaskmaster(ticketStatus.status);
						...
						let taskLastUpdated =
							(taskItem.metadata && taskItem.metadata.lastStatusUpdate) || null;

						// Initialize task timestamp if missing
						if (!taskLastUpdated && jiraLastUpdated) {
							debugLog(
								`No timestamp in TaskMaster, initializing from Jira's updated time: ${jiraLastUpdated}`
							);
							...
							stats.tasksWithTimestampsAdded =
								(stats.tasksWithTimestampsAdded || 0) + 1;
						}

						debugLog(
							`${isSubtaskItem ? 'Subtask' : 'Task'} ${taskItem.id} status: ${currentTaskStatus} (last updated: ${taskLastUpdated || 'never'})`
						);
						debugLog(
							`Jira status: ${jiraStatusInTaskmaster} (last updated: ${jiraLastUpdated || 'never'})`
						);
						...
						if (currentTaskStatus !== jiraStatusInTaskmaster) {
							...
							if (
								taskLastUpdated &&
								(!jiraLastUpdated ||
									new Date(taskLastUpdated) > new Date(jiraLastUpdated))
							) {
								debugLog(
									`TaskMaster has more recent update (${taskLastUpdated}), updating Jira ticket`
								);
								...
								try {
									...
									if (updated) {
										customLog.success(
											`Updated Jira ticket ${ticketIdentifier} status to match TaskMaster ${isSubtaskItem ? 'subtask' : 'task'} ${taskItem.id} status: ${currentTaskStatus}`
										);
										stats.ticketsUpdated = (stats.ticketsUpdated || 0) + 1;
									} else {
										customLog.error(
											`Failed to update Jira ticket ${ticketIdentifier} status, but will preserve local ${isSubtaskItem ? 'subtask' : 'task'} status`
										);
									}
								} catch (error) {
									customLog.error(
										`Error updating Jira ticket status: ${error.message}`
									);
								}
							}
							// Case 2: Jira was updated more recently than TaskMaster - update TaskMaster
								...
								jiraLastUpdated &&
								(!taskLastUpdated ||
									new Date(jiraLastUpdated) > new Date(taskLastUpdated))
							) {
								debugLog(
									`Jira has more recent update (${jiraLastUpdated}), updating TaskMaster ${isSubtaskItem ? 'subtask' : 'task'}`
								);
								...
								if (isSubtaskItem) {
									customLog.success(
										`Updated subtask ${taskItem.id} status to ${jiraStatusInTaskmaster} from Jira ticket ${ticketIdentifier}`
									);
									stats.subtasksUpdated++;
								} else {
									customLog.success(
										`Updated task ${taskItem.id} status to ${jiraStatusInTaskmaster} from Jira ticket ${ticketIdentifier}`
									);
									stats.tasksUpdated++;
								}
							}
							// Case 3: Cannot determine which is more recent - use TaskMaster as source of truth
								...
								debugLog(
									`Cannot determine which status is more recent, using TaskMaster as source of truth`
								);
								...
								try {
									...
									if (updated) {
										customLog.success(
											`Updated Jira ticket ${ticketIdentifier} status to match TaskMaster status: ${currentTaskStatus}`
										);
										stats.ticketsUpdated = (stats.ticketsUpdated || 0) + 1;
									}
								} catch (error) {
							...
							debugLog(
								`${isSubtaskItem ? 'Subtask' : 'Task'} and Jira status match (${currentTaskStatus}), no update needed`
							);
						}
					} catch (error) {
						customLog.error(
							`Error synchronizing status for ${isSubtaskItem ? 'subtask' : 'task'} ${taskItem.id}: ${error.message}`
						);
					}
				}
				...
				if (task.subtasks && task.subtasks.length > 0 && ticketId) {
						...
						try {
							...
							if (!subtaskTicketId) {
								...
								try {
									...
									if (subtaskTicketId) {
										...
										try {
											const ticketStatus =
												await ticketingSystem.getTicketStatus(
													subtaskTicketId,
													projectRoot
												);
											if (ticketStatus && ticketStatus.updated) {
												...
												if (!subtask.metadata.lastStatusUpdate) {
													subtask.metadata.lastStatusUpdate =
														ticketStatus.updated;
													stats.tasksWithTimestampsAdded++;
													debugLog(
														`Added timestamp from Jira to subtask ${subtask.id}: ${ticketStatus.updated}`
													);
												}
											}
										} catch (e) {
											debugLog(
												`Could not get ticket status for subtask timestamp: ${e.message}`
											);
										}
							...
							if (!subtaskTicketId) {
								...
								try {
									...
									customLog.info(
										`Creating subtask with title: ${subtaskTitle}`
									);
									...
									const result = await ticketingSystem.createStory(
										subtaskData,
										projectRoot
									);
					...
					if (task.subtasks && task.subtasks.length > 0) {
						...
						// Warn if we don't have a valid ticket ID for the parent task
						if (!parentTicketId) {
							customLog.warn(
						...
						if (!parentTicketId) {
							customLog.error(`Synchronization error: ticketId is not defined`);
							customLog.warn(`Ticket synchronization skipped: Error: ticketId is not defined`);
							break; // Skip all subtasks if parent ticket ID is missing
						}
						...
						for (const subtask of task.subtasks) {
							try {
								...
								if (!subtaskTicketId) {
									customLog.warn(
										`Subtask ${subtask.id} has no ticket ID. Skipping synchronization.`
									);
									continue; // Skip to the next subtask
								}
								...
								try {
									...
									if (ticketStatus) {
										jiraSubtaskStatus =
											ticketingSystem.mapTicketStatusToTaskmaster(
												ticketStatus.status
											);
										...
										// Get last update timestamps
										const jiraLastUpdated = ticketStatus.updated || null;
										let subtaskLastUpdated =
											subtask.metadata.lastStatusUpdate || null;

										// If subtask doesn't have a timestamp but Jira does, populate it
										if (!subtaskLastUpdated && jiraLastUpdated) {
											debugLog(
												`No timestamp in subtask, initializing from Jira's updated time: ${jiraLastUpdated}`
											);
										...
										if (subtaskStatus !== jiraSubtaskStatus) {
											...
											if (
												subtaskLastUpdated &&
												(!jiraLastUpdated ||
													new Date(subtaskLastUpdated) >
														new Date(jiraLastUpdated))
											) {
												debugLog(
													`Subtask has more recent update (${subtaskLastUpdated}), updating Jira ticket`
												);

												// Use the updated updateTicketStatus with subtask data
												const updated =
													await ticketingSystem.updateTicketStatus(
														subtaskTicketId,
														subtaskStatus,
														projectRoot,
														subtask
												...
												if (updated) {
													customLog.success(
														`Updated Jira ticket ${subtaskTicketId} status to match subtask ${subtask.id} status: ${subtaskStatus}`
													);
													stats.ticketsUpdated++;
												} else {
													customLog.error(
														`Failed to update Jira ticket ${subtaskTicketId} status, but will preserve local subtask status`
													);
												}
											}
											// Case 2: Jira was updated more recently than subtask - update subtask
												...
												debugLog(
													`Jira has more recent update (${jiraLastUpdated}), updating subtask`
												);
												...
												customLog.success(
													`Updated subtask ${subtask.id} status to ${jiraSubtaskStatus} from Jira ticket ${subtaskTicketId}`
												);
												stats.subtasksUpdated++;
											}
											// Case 3: Cannot determine which is more recent - use subtask as source of truth
												...
												debugLog(
													`Cannot determine which status is more recent, using subtask as source of truth`
												);
												...
												if (updated) {
													customLog.success(
														`Updated Jira ticket ${subtaskTicketId} status to match subtask status: ${subtaskStatus}`
													);
													stats.ticketsUpdated++;
												}
											}
											...
											debugLog(
												`Subtask and Jira status match (${subtaskStatus}), no update needed`
											);
										}
									}
								} catch (ticketError) {
									customLog.error(
										`Error getting ticket status for subtask ${subtask.id}: ${ticketError.message}`
									);
								}
							} catch (syncError) {
								customLog.error(
									`Error synchronizing subtask ${subtask.id}: ${syncError.message}`
								);
								stats.errors++;
							}
						}
		...
		// Fetch all tickets from the ticketing system
		const allTickets = await fetchAllTicketsFromSystem(
			ticketingSystem,
			projectRoot,
			customLog,
			debugLog
		...
		for (const ticket of allTickets) {
			...
			if (newTask.isSubtask && newTask.parentKey) {
				...
				if (parentTaskId) {
					...
					customLog.info(
						`Adding new subtask ${subtaskId} from ticket ${ticketId} to task ${parentTaskId}`
					);
					stats.subtasksUpdated++;
				} else {
		...
		if (tasksToAdd.length > 0) {
			...
			customLog.success(
				`Added ${tasksToAdd.length} new tasks from ticketing system`
			);
		}
		...
		for (const [parentId, subtasks] of subtasksToAdd.entries()) {
			...
			if (parentTask) {
				...
				customLog.success(
					`Added ${subtasks.length} subtasks to task ${parentId}`
				);
			}
		}
