...
async function initializeMappings(options = {}, explicitRoot = null) {
	...
	if (currentSystem && currentSystem !== 'none') {
		log(
			'success',
			`Mapping files initialized for ${currentSystem} and other supported ticketing systems`
		);
		log(
			'info',
			`You can customize the mappings in the taskmaster/mappings/ directory`
		);
	} else {
		log(
			'info',
			`Default mapping files created. Enable a ticketing system in your .taskmasterconfig file to use them`
		);
	}
