...
import { log, findProjectRoot, readJSON, writeJSON, isSilentMode } from '../utils.js';
...
class TicketingSyncService {
    ...
    async initialize(projectRoot = null) {
        try {
            ...
            if (this.isEnabled) {
                this.ticketingInstance = await getTicketingInstance(null, this.projectRoot);
    ...
    async syncSubtasks(subtasks, parentTask, tasksPath, projectRoot) {
        ...
        for (const subtask of subtasks) {
            const ticket = await this.syncSubtask(subtask, parentTask, tasksPath, projectRoot);
            if (ticket && ticket.success) {
                results.push(ticket);
            }
        }

        this._log('info', `Synced ${results.length}/${subtasks.length} subtasks to ticketing system`);
        return results;
    }
    ...
    async syncTask(task, tasksPath, projectRoot) {
        try {
            ...
            log('debug', `[TICKETING_SERVICE] Syncing task ${task.id} with ticketing system`);
            ...
            if (!updatedTask.metadata?.refId) {
                ...
                if (refId) {
                    ...
                    log('debug', `[TICKETING_SERVICE] Generated refId ${refId} for task ${task.id}`);
                }
            }
            ...
            const ticketingIssue = await this.ticketingInstance.createStory(ticketData, projectRoot);
            ...
            if (ticketingIssue && ticketingIssue.key) {
                ...
                if (typeof this.ticketingInstance.storeTicketId === 'function') {
                    updatedTask = this.ticketingInstance.storeTicketId(updatedTask, ticketingIssue.key);
                }

                // Update the task in the file
                await this._updateTaskInFile(updatedTask, tasksPath);

                log('success', `[TICKETING_SERVICE] Created ticket ${ticketingIssue.key} for task ${task.id}`);
                return { success: true, ticketKey: ticketingIssue.key };
            } else {
                log('warn', `[TICKETING_SERVICE] Failed to create ticket for task ${task.id}`);
                return { success: false, error: 'Failed to create ticket' };
            }

        } catch (error) {
            log('error', `[TICKETING_SERVICE] Error syncing task ${task.id}: ${error.message}`);
    ...
    async syncSubtask(subtask, parentTask, tasksPath, projectRoot) {
        try {
            ...
            log('debug', `[TICKETING_SERVICE] Syncing subtask ${parentTask.id}.${subtask.id} with ticketing system`);

            // Get parent ticket key
            const parentTicketKey = this.ticketingInstance.getTicketId(parentTask);
            
            if (!parentTicketKey) {
                log('warn', `[TICKETING_SERVICE] Parent task ${parentTask.id} has no ticket key, cannot create subtask ticket`);
            ...
            const ticketingIssue = await this.ticketingInstance.createStory(subtaskData, projectRoot);
            ...
            if (ticketingIssue && ticketingIssue.key) {
                ...
                log('success', `[TICKETING_SERVICE] Created ticket ${ticketingIssue.key} for subtask ${parentTask.id}.${subtask.id}`);
                return { success: true, ticketKey: ticketingIssue.key };
            } else {
                log('warn', `[TICKETING_SERVICE] Failed to create ticket for subtask ${parentTask.id}.${subtask.id}`);
                return { success: false, error: 'Failed to create subtask ticket' };
            }

        } catch (error) {
            log('error', `[TICKETING_SERVICE] Error syncing subtask ${parentTask.id}.${subtask.id}: ${error.message}`);
            return { success: false, error: error.message };
        }
    }
    ...
    async syncMultipleTasks(tasks, tasksPath, projectRoot) {
        try {
            ...
            if (!this.isReady()) {
                const initResult = await this.initialize(projectRoot);
                if (!initResult) {
                    return { success: false, created: 0, errors: ['Ticketing service not available'] };
                }
            }
            ...
            for (const task of tasks) {
                ...
                if (task.subtasks && task.subtasks.length > 0) {
                    for (const subtask of task.subtasks) {
                        ...
                        const subtaskResult = await this.syncSubtask(subtask, task, tasksPath, projectRoot);
                        if (subtaskResult.success) {
                            created++;
                        } else {
                            errors.push(`Subtask ${task.id}.${subtask.id}: ${subtaskResult.error}`);
                        }
                    }
                }
            }

            log('info', `[TICKETING_SERVICE] Sync completed: ${created} tickets created, ${errors.length} errors`);
            return { success: true, created, errors };

        } catch (error) {
            log('error', `[TICKETING_SERVICE] Error syncing multiple tasks: ${error.message}`);
    ...
    async updateTaskStatus(taskId, newStatus, tasksPath, projectRoot) {
        try {
            ...
            if (!ticketKey) {
                log('debug', `[TICKETING_SERVICE] Task ${taskId} has no ticket, skipping status update`);
                return { success: true }; // Not an error, just no ticket to update
            }

            // Update ticket status
            await this.ticketingInstance.updateTicketStatus(ticketKey, newStatus, projectRoot);
            
            log('debug', `[TICKETING_SERVICE] Updated ticket ${ticketKey} status to ${newStatus}`);
            return { success: true };

        } catch (error) {
            log('error', `[TICKETING_SERVICE] Error updating task status: ${error.message}`);
            return { success: false, error: error.message };
        }
    }
    ...
    async _updateSubtaskInFile(updatedSubtask, parentTaskId, tasksPath) {
        ...
        if (parentTask && parentTask.subtasks) {
            const subtaskIndex = parentTask.subtasks.findIndex(st => st.id === updatedSubtask.id);
            if (subtaskIndex !== -1) {
